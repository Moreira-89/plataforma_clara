...
from app.ingestion.csv_processor import (
...
def test_todo_valor_do_resultado_e_tipo_nativo_do_python(escrever_csv, linha_aporte_valida):
    """
    Contrato com quem persiste: nenhum valor pode ser escalar do NumPy/Pandas.

    O juiz aqui era o `psycopg2.extensions.adapt`, porque todo valor virava
    parâmetro de INSERT. Com a saída do Postgres o destino mudou — Firestore e
    BigQuery, ambos serializando para JSON —, mas o invariante é o mesmo e a
    armadilha também: `numpy.int64` e `pandas.NA` se parecem com tipos nativos,
    passam em `isinstance` distraído e quebram na serialização.
    """
    caminho = escrever_csv([dict(linha_aporte_valida, codigo_identificacao_isin="")])
    registro = processar_arquivo_csv(caminho).to_dict(orient="records")[0]

    nativos = (type(None), bool, int, float, str, datetime.date, datetime.datetime)
    for coluna, valor in registro.items():
        assert type(valor) in nativos, (
            f"{coluna}={valor!r} é {type(valor).__module__}.{type(valor).__name__}, "
            "não um tipo nativo do Python"
        )
